...
def test_writes_a_folder_per_conversation(archive_dir: Path) -> None:
    folder = ArchiveWriter(archive_dir).write(conversation()).folder
...
def test_an_undated_conversation_is_still_filed(archive_dir: Path) -> None:
    folder = ArchiveWriter(archive_dir).write(conversation(created=None)).folder
...
def test_a_title_with_a_colon_does_not_break_the_front_matter(
    ...
    folder = (
        ArchiveWriter(archive_dir)
        .write(conversation(title="Bread: a beginner's guide"))
        .folder
    )
    text = (folder / "conversation.md").read_text(encoding="utf-8")
...
def test_a_hostile_title_cannot_escape_the_archive(
    ...
    folder = ArchiveWriter(archive_dir).write(conversation(title=hostile_title)).folder
...
def test_a_very_long_title_is_truncated(archive_dir: Path) -> None:
    folder = ArchiveWriter(archive_dir).write(conversation(title="a" * 500)).folder
...
def test_re_importing_the_same_conversation_updates_it(archive_dir: Path) -> None:
    ...
    first = writer.write(conversation()).folder
    second = writer.write(conversation()).folder
...
def test_two_different_conversations_with_the_same_name_both_survive(
    ...
    first = writer.write(conversation(source_id="one")).folder
    second = writer.write(conversation(source_id="two")).folder
